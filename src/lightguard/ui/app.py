...
def create_app(
    ...
    @app.route("/network")
    def network_monitor():
        """Live per-process network anomaly view (v2 module)."""
        from pathlib import Path as _Path

        detector_path = _Path("data") / "network" / "detector.joblib"
        ...
        try:
            from lightguard.network.collector import collect_snapshot
            from lightguard.network.detector import load_detector, score
            ...
            if snap.empty:
                ...
                scored = score(snap, detector)
                explained = explain_snapshot(scored, detector)
                rows = explained.to_dict(orient="records")
