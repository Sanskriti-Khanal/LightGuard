...
# ── /network (v2 network monitor) ─────────────────────────────────────────────

class TestNetworkPage:
    """Tests for GET /network — mocks all v2 network imports."""

    def test_no_detector_returns_200(self, client, tmp_path, monkeypatch):
        # No detector file → friendly "no baseline" page, not a crash
        monkeypatch.chdir(tmp_path)
        r = client.get("/network")
        assert r.status_code == 200
        assert b"No baseline" in r.data

    def test_no_detector_shows_demo_hint(self, client, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        r = client.get("/network")
        assert b"network_demo" in r.data

    def test_with_detector_returns_200(self, client, tmp_path, monkeypatch):
        import pandas as pd
        from unittest.mock import MagicMock, patch

        # Create a fake detector file so the route proceeds past the existence check
        det_dir = tmp_path / "data" / "network"
        det_dir.mkdir(parents=True)
        (det_dir / "detector.joblib").write_text("fake")
        monkeypatch.chdir(tmp_path)

        fake_df = pd.DataFrame([{
            "pid": 1, "proc_name": "chrome", "anomaly_score": 10,
            "label": "NORMAL", "conn_count": 5.0, "unique_remote_ips": 2.0,
            "unique_remote_ports": 2.0, "rare_port_count": 0.0,
            "conn_per_min": 0.0, "reasons": [],
        }])

        with patch("lightguard.network.detector.load_detector", return_value=MagicMock()), \
             patch("lightguard.network.collector.collect_snapshot", return_value=fake_df), \
             patch("lightguard.network.detector.score", return_value=fake_df), \
             patch("lightguard.network.explain.explain_snapshot", return_value=fake_df):
            r = client.get("/network")

        assert r.status_code == 200
        assert b"chrome" in r.data

    def test_anomalous_row_highlighted(self, client, tmp_path, monkeypatch):
        import pandas as pd
        from unittest.mock import MagicMock, patch
        ...
        fake_df = pd.DataFrame([{
            "pid": 99, "proc_name": "evil.exe", "anomaly_score": 95,
            "label": "ANOMALOUS", "conn_count": 200.0, "unique_remote_ips": 50.0,
            "unique_remote_ports": 30.0, "rare_port_count": 25.0,
            "conn_per_min": 0.0, "reasons": ["Using ports not normally used by this app"],
        }])
        ...
        assert b"ANOMALOUS" in r.data
        assert b"evil.exe" in r.data
        assert b"Using ports not normally" in r.data

    def test_normal_row_not_highlighted_red(self, client, tmp_path, monkeypatch):
        import pandas as pd
        from unittest.mock import MagicMock, patch
        ...
        fake_df = pd.DataFrame([{
            "pid": 1, "proc_name": "firefox", "anomaly_score": 5,
            "label": "NORMAL", "conn_count": 3.0, "unique_remote_ips": 1.0,
            "unique_remote_ports": 1.0, "rare_port_count": 0.0,
            "conn_per_min": 0.0, "reasons": [],
        }])
        ...
        assert b"NORMAL" in r.data
        assert b"firefox" in r.data

    def test_empty_snapshot_shows_empty_state(self, client, tmp_path, monkeypatch):
        import pandas as pd
        from unittest.mock import MagicMock, patch
        ...
        with patch("lightguard.network.detector.load_detector", return_value=MagicMock()), \
             patch("lightguard.network.collector.collect_snapshot", return_value=pd.DataFrame()):
            r = client.get("/network")

        assert r.status_code == 200
        assert b"No active" in r.data

    def test_nav_link_present_on_dashboard(self, client):
        r = client.get("/")
        assert b"/network" in r.data

    def test_dashboard_network_card_links_to_network(self, client):
        r = client.get("/")
        assert b"network_monitor" in r.data or b"/network" in r.data
